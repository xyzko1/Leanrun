...
(function ($) {
    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                 ...
                 */
                cbiconpath: getInfoTop().contentPath + "/Content/scripts/plugins/tree/images/icons/",
        ...
        function searchTree() {
            var txtSearch = me.find("#txtTreeSearch");
            txtSearch.bind("keyup", function (event) {
                if (event.keyCode == 13) {
                    var districtName = txtSearch.val();
                    if (dfop.searchUrl) {
                        $.ajax({
                            type: dfop.method,
                            url: dfop.searchUrl,
                            ...
                            success: function (data) {
                                var tree = me.find(".bbit-tree-bwrap");
                                tree.remove();
                                var _html = [];
                                var arr = JSON.parse(data);
                                buildtree2(arr, _html);
                                me.addClass("bbit-tree").append(_html.join(""));
                                InitEvent(me);
                            },
                            error: function (e) {
                                dialogMsg("服务端未响应。", -1);
        ...
        function buildtree(data, ht) {
            //搜索功能
            if (dfop.isSearch) {
                ht.push("<div id='treeSearch'><input id='txtTreeSearch' type='text' class='form-control' placeholder='请输入要查询关键字' /></div>");
            }
            else {
                ht.push("<div id='treeSearch' style='display:none'><input id='txtTreeSearch' type='text' class='form-control' placeholder='请输入要查询关键字' /></div>")
            }
        ...
        //region 
        function buildtree2(data, ht) {
            ////搜索功能
            //if (dfop.isSearch) {
            //    ht.push("<div><input id='txtTreeSearch' type='text' class='form-control' placeholder='请输入要查询关键字' /></div>");
            //}
            //else {
            //    ht.push("<div style='display:none'><input id='txtTreeSearch' type='text' class='form-control' placeholder='请输入要查询关键字' /></div>")
            //}
            ht.push("<div class='bbit-tree-bwrap'>"); // Wrap ;
            ht.push("<div class='bbit-tree-body " + id + "'>"); // body ;
            ht.push("<ul class='bbit-tree-root ", dfop.theme, "'>"); //root
