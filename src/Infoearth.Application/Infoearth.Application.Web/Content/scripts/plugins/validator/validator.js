...
$.fn.Validform = function () {
    ...
    $(this).find("[isvalid=yes]").each(function () {
        ...
        if (checkexpession != undefined) {
            ...
            switch (checkexpession) {
                ...
                case "LenDoubleOrNull":
                    {
                        if (!isLenDoubleOrNull(value, $(this).attr("length"))) {
                            Validatemsg = errormsg + "小数位必须小于等于" + $(this).attr("length") + "！\n";
                            Validateflag = false;
                            ValidationMessage($(this), Validatemsg); return false;
                        }
                        break;
    ...
    //验证小数位长度
    function isLenDoubleOrNull(obj, len) {
        var arr = len.split(",");
        var int = arr[0];
        var point = 2;
        if (arr.length > 1) {
        ...
        //var zz = " /^\d{1," + int + "}(\.\d{1," + point + "})?$/";
        eval("reg=/^\\d{1," + int + "}(\\.\\d{1," + point + "})?$/;");
        var controlObj = $.trim(obj);
        if (controlObj == null || controlObj == undefined || controlObj.length == 0) {
            return true;
        }
