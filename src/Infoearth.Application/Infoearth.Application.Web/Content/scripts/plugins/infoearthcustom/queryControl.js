    ...
    $.fn.queryControl = function (settings) {
        ...
        function GetData() {
            ...
            for (var i = 2; i <= countDiv.length - 1; i++) {
                ...
                data.push({ QueryField: arrwhere2[0], QueryFieldName: me.find("#switchWhere" + i + " option:selected").text(), QueryTJ: convertLogic((me.find("#logic")).val()), ControlType: arrwhere2[1], QueryConChar: (me.find("#Condition" + i)).val(), IsDic: isDic, DicCode: dicCode });
            }
            return data;
        }
