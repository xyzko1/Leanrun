...
(function ($, learun) {
    ...
    $.fn.comboBox = function (opt) {//下拉框
        ...
        var opt = $.extend({
            ...
            EnCode: null,  //字典值（用于封装combox和力软的字典项）
        }, opt);
        var dom = {
            rendering: function () {
            ...
            loadData: function () {
                if (!!opt.url) {
                    ...
                    var data = topInfo.learun.data.get(["dataItem", opt.EnCode]);
                    var dataDMLX = [];
                    for (var item in data) {
                        //遍历非主键的key值
                        if (item.length != 36) {
                            dataDMLX.push({ text: data[item], value: item, complete: true, showcheck: false });
                        }
                    }
