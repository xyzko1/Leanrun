    ...
    $.extend(learun, {
        ...
        loading: function (ops) {//加载动画显示与否
            ...
            if (!!ops.text) {
                ...
                getInfoTop().$("#loading_manage").html("正在加载…");
            }
        ...
        //带url的Confirm事件
        dialogOpenConfirm: function (opt) {
            learun.loading({ isShow: true });
            var opt = $.extend({
                id: null,
                title: '系统窗口',
        ...
        setForm: function (opt) {
            ...
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    ...
                    if (opt.authToken) {
                        request.setRequestHeader("Authorization", opt.authToken);
                    }
                    learun.loading({ isShow: true });
                },
                complete: function () {
        ...
        removeForm1: function (opt) {
            var opt = $.extend({
                msg: "注：此操作将删除该项目下关联的各阶段信息，您确定要删除吗？",
                loading: "正在删除数据...",
                url: "",
                param: [],
                type: "post",
