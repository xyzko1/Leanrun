...
namespace iTelluro.Busness.WebApi
{
    /// <summary>
    ///版 本 V1.0
    ///Copyright (c) 2010-2015 武汉地大信息股份有限公司　　　　　　　　　　　　　　　　　　　　　　　　　　
    ///创建人：刘晓雷
    ///日 期：2015/11/25
    ///描 述：NPOI Excel泛型操作类
    public class ExcelHelperEx<T>
    {
        ...
        public static void ExcelDownload(List<T> lists, ExcelConfig excelConfig, bool isFilter = false)
        {
            ...
            if (isFilter)
            {
                curContext.Response.BinaryWrite(ExportMemoryStreamFilt(lists, excelConfig).GetBuffer());
            }
            else
            {
        ...
        public static string ExcelCreate(List<T> lists, ExcelConfig excelConfig, bool isFilter = false)
        {
            try
            {
                ...
                if (isFilter)
                {
                    myByte = ExportMemoryStreamFilt(lists, excelConfig).GetBuffer();
                }
                else
                {
                    myByte = ExportMemoryStream(lists, excelConfig).GetBuffer();
                }
                ...
                string filePath = strPath + System.Guid.NewGuid() + Path.GetExtension(excelConfig.FileName);
        ...
        public static MemoryStream ExportMemoryStreamFilt(List<T> lists, ExcelConfig excelConfig)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            ISheet sheet = workbook.CreateSheet();
            Type type = typeof(T);
            ...
            foreach (PropertyInfo column in properties)
            {
                if (excelConfig.ColumnEntity.Select(p => p.Column).Contains(column.Name))
                {
                    ICellStyle columnStyle = workbook.CreateCellStyle();
                    columnStyle.Alignment = HorizontalAlignment.Center;
                    arrColWidth[i] = Encoding.GetEncoding(936).GetBytes(column.Name).Length;
                ...
                foreach (PropertyInfo column in properties)
                {
                    if (excelConfig.ColumnEntity.Select(p => p.Column).Contains(column.Name))
                    {
                        ...
                        if (excelConfig.IsAllSizeColumn)
                        {
                            ...
                            int intTemp = Encoding.GetEncoding(936).GetBytes(drValue).Length;
                            if (intTemp > arrColWidth[ordinal])
                            {
                                arrColWidth[ordinal] = intTemp;
                            }
                            #endregion
                        }
            ...
            #region 设置列宽
            for (int index = 0; index < arrColWidth.Length; index++)
            {
                if ((arrColWidth[index] + 1) * 256 >= 10000)
                {
